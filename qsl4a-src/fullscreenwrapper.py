...
import abc,pickle,json,sys,time,os,hashlib
...
class EventHandler(object):
    ...
    fullscreenwrapper2 module pre-defines click_EventHandler, itemclick_EventHandler, itemlongclick_EventHandler and key_EventHandler which are
...
class itemlongclick_EventHandler(EventHandler):
    '''
    predefined itemlongclick event handler for use with  Views
    
    This is the event handler to typically associate with ListView. You only need to pass the ListView to link the itemlongclick event 
    to & the handler function & rest of event handler initialization is handled automatically
    ''' 
...
class View(object):
    ...
    def add_event(self, eventhandler):
        ...
        You would typically add one of click_EventHandler or itemclick_EventHandler or itemlongclick_EventHandler (for List Views)
        to a view
        '''
    ...
    def set_listitems(self,listitems,isHtml=False,listType=0):
        '''
        sets a list for a ListView. Takes a list of string as input ,
        string can be plain text ( isHtml = False ) or HTML ( isHtml = True )
        listType default is SIMPLE list (0)
        '''
        FullScreenWrapper2App.set_list_contents(self.view_id, listitems, isHtml, listType)
...
class Layout(object):
    ...
    and on_close(self). in your MyLayout.__init__(), you MUST include a call to super(MyLayout,self).__init__(xml, title, theme)
    ...
    def __init__(self,xml,title=None,theme=None):
        '''
        creates a layout and stes its xml,title,theme, initializes the views collection
        ...
        '''
        self.uid = hashlib.md5(str(str(title)+str(os.getpid())+str(time.time())).encode("UTF-8")).hexdigest()
        self.title = title
        self.xml = xml
        if theme==None:
...
class FullScreenWrapper2App(object):
    ...
    #Internal Signal for exit FullScreenWrapper2App or insert operation before eventPoll function .
    #SIGNAL_DATA stores signals of exit or insert function . 
    #EVENT_DATA stores event queue
    SIGNAL_DATA=[]
    EVENT_DATA=[]
    ...
    def show_layout(cls, layout, show_mode = SHOW_LAYOUT_REPLACING_CURRENT):
        '''
        This will show the layout, set the title and theme, clean & re-populate layout's views BaseDict collection & call the Layout.on_show() 
        ...
        if show_mode in (0,1,2): # show_mode == cls.SHOW_LAYOUT_PUSH_OVER_CURRENT or show_mode == cls.SHOW_LAYOUT_REPLACING_CURRENT or show_mode == cls._SHOW_LAYOUT_POP_CURRENT
            _layout = cls._layouts
            _android = cls.get_android_instance()
            ...
            _android.fullShow(layout.xml,layout.title,layout.theme)
            
            viewsdict = _android.fullQuery().result
            layout._reset()
    ...
    def insert_function(cls, function,*args,**kwargs):
        '''
        Insert operations before FullScreenWrapper2App eventloop function
        '''
    ...
    def get_event(cls):
        '''
        Check FullScreenWrapper2App has a waiting event or not .
        '''
        cls.EVENT_DATA.extend(cls.get_android_instance().eventPoll())
    ...
    def eventloop(cls):
        ...
        while(True):
            ...
            evt=cls.EVENT_DATA
            evt.extend(cls.get_android_instance().eventPoll().result)
            while len(evt)>0:
                eventdata=evt.pop(0)
                
                #this corrects an eventpost issue where an extra "" wraps the json
                try:
                    if type(eventdata["data"]) != dict:
    ...
    @classmethod
    def set_list_contents(cls, *a, **k):
        return cls.get_android_instance().fullSetList(*a,**k)
    ...
    def get_property_value(cls, id, property):
        ...
        return cls.get_android_instance().fullGetProperty(id,property).result