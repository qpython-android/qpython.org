...
- **[QPython](qpython-x.md)** – 具有 AI 功能的主要版本，可在 Google Play 和应用商店下载
- **[QPython+](qpython-x.md)** – 面向贡献者的社区开源版本
...
- **扩展包安装** - QPYPI 和 pip 支持以扩展功能
- **内置编辑器** - 语法高亮和代码编辑
- **多种运行模式** - 除了控制台程序外，还支持Android 原生UI（通过QSL4A接口），支持Pygame / Turtle / Tkinter等运行方式

---

## 1. 仪表盘

![QPython 启动](static/qpy_dashboard.jpg)
...
![QPython 控制台](static/terminal_demo.jpg)
...
![QPython 编辑器](static/qpy_editor.jpg)

除了 内置Python 语法高亮，显示行号外，

编辑器底部的功能依次为（从左到右）：

- 切换到快捷输入（比如包括def / if / else / elif / class 等关键词的输入）
- 锁定，避免输入
- 缩进控制
- 保存
- 运行
...
while True:
    ...
    time.sleep(60)
```
如果运行带GUI的QSL4A程序，不希望显示控制台信息，则推荐用该模式
